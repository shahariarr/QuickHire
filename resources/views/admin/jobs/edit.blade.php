        ...
        <form action="{{ route('admin.jobs.update', $job->id) }}" method="POST" class="space-y-5" novalidate>
            ...
            <div>
                ...
                <textarea name="description" rows="8"
                          class="w-full px-4 py-2.5 border {{ $errors->has('description') ? 'border-red-400' : 'border-gray-200' }} rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand focus:border-transparent resize-none">{{ old('description', $job->description) }}</textarea>
